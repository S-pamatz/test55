...
@pytest.fixture
def new_user():
    user = Student(username='sakire', email='sakire@wsu.edu',
                   firstname='Sakire', lastname='Arslan Ay', address='Pullman, WA')
...
def init_database(request, test_client):
    ...
    if Major.query.count() == 0:
        ...
        db.session.commit()
    # add a user
    user1 = Student(username='sakire', email='sakire@wsu.edu',
                    firstname='Sakire', lastname='Arslan Ay', address='Pullman, WA')
...
def test_register(request, test_client, init_database):
    ...
    response = test_client.post('/register',
                                data=dict(username='john', email='john@wsu.edu', password="bad-bad-password",
                                          password2="bad-bad-password", firstname='John', lastname='Yates', address='Pullman, WA'),
...
def test_invalidlogin(request, test_client, init_database):
    ...
    response = test_client.post('/login',
                                data=dict(username='sakire',
                                          password='12345', remember_me=False),
                                follow_redirects=True)
    assert response.status_code == 200
...
def test_login_logout(request, test_client, init_database):
    ...
    response = test_client.post('/login',
                                data=dict(username='sakire',
                                          password='1234', remember_me=False),
                                follow_redirects=True)
    assert response.status_code == 200
...
def test_createclass(request, test_client, init_database):
    ...
    response = test_client.post('/createclass',
                                data=dict(
                                    coursenum='355', title='Programming Languages', major='CptS'),
                                follow_redirects=True)
    assert response.status_code == 200
